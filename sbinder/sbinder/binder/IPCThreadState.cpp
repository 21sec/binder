...
restart:
    ...
    if (gShutdown) {
		TOSLOGF("IPCThreadState::self return null since gShutdown\n");
		return NULL;
    }
    ...
    if (!gHaveTLS) {
        if (pthread_key_create(&gTLS, threadDestructor) != 0) {
			...
			TOSLOGF("IPCThreadState::self return null since pthread_key_create failed\n");
            return NULL;
        }
        gHaveTLS = true;
    }
...
{
	TOSLOGW("IPCThreadState::shutdown\n");
