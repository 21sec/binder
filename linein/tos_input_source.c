...
#endif        
    {
//		DBG_SRC("It's not tv input =0x%x\n",linein);
		for(i = 0; i < E_INPUT_NUM; ++i)
		{
			if(input == (EN_API_SOURCE_INPUT)ui_input_soure_mapping[i][0])
			{
	...
	if( act == EN_ACT_EXEC_SAVE)
	{
		...
		if(linein != FPI_LINEIN_INVALID)
		{
			...
			{
				DBG_SRC_ERROR("app_set_input_source fail\n");
			}
		}
	...
	else if( act == EN_ACT_EXEC )
	{
		fpi_info("%s EN_ACT_EXEC! s_current_linein is not equal to the value of db \n",__FUNCTION__);

		/*if current source is CATV, source should be released before change linein*/
		if(app_is_current_linein_itv())
		{
			itv_platform_offItv();
		}
		app_set_pre_input_source(linein);
		ret = fpi_linein_changeto(linein) ;
		app_notify_event(APP_NOTIFY_LINEIN_CHANGE_FINISH, 0, 0);
	}
	else if( act == EN_ACT_SAVE )
	{
		fpi_trace("EN_ACT_EXEC\n");
		if (fpi_true == app_save_db_current_linein(linein))
		{
			ret = FPI_ERROR_SUCCESS;
		}
	}
	...
	{
		fpi_warn("app_set_input_source_v2\n");
...
fpi_error tos_source_set_input(EN_API_SOURCE_INPUT input,EN_ACT_CTRL act)
{
	...
	fpi_info("tos_source_set_input, input=0x%x,act=%d, _time=%d ms\n",input,act, ms );
...
{
	*isitv = app_is_current_linein_itv();
...
fpi_error tos_source_get_input(EN_SOURCE_SAVE_TYPE type,EN_API_SOURCE_INPUT *value)
{
	...
	fpi_trace(" enter %s, line =%d,type = %d\n",__FUNCTION__,__LINE__,type);
	if( type == EN_SOURCE_CURRENT )
	{
		linein = app_get_input_sources();
		fpi_trace("app_get_input_sources =%d \n",linein);
	}
	else if( type == EN_SOURCE_LASTEST )
	{
		linein = app_get_lastest_input_source();
		fpi_trace("app_get_lastest_input_source =%d \n",linein);
	}
	else
	{
		 linein = fpi_get_target_linein();
		fpi_trace("fpi_get_target_linein =%d \n",linein);
	}
...
fpi_error tos_source_is_insert(EN_API_SOURCE_INPUT inSource, int *state)
{
	...
	else
	{
		...
		fpi_trace("tos_source_is_insert success, isInsert = %d\n", *state);
	}
...
fpi_error tos_source_get_hdmi_edid_ver(EN_API_SOURCE_INPUT input, int32_t *ver)
{
    ...
    switch(input)
    {
        ...
        default:
        {
            DBG_SRC_ERROR(" tos_source_get_hdmi_edid_ver error! input = %d \n", input);
            return FPI_ERROR_FAIL;
        }
        break;
    }

    ret = app_get_db_hdmi_version(&hdmi_ver, linein);
    if(fpi_true == ret)
    {
        *ver = hdmi_ver;
        DBG_SRC(" tos_source_get_hdmi_edid_ver, input = %d, ver = %d \n",input, *ver);
        return FPI_ERROR_SUCCESS;
    }
    else
    {
        DBG_SRC_ERROR(" tos_source_get_hdmi_edid_ver error!\n");
        return FPI_ERROR_FAIL;
    }
}
...
fpi_error tos_source_set_hdmi_edid_ver(EN_API_SOURCE_INPUT input, int32_t ver)
{
    ...
    ret = fpi_input_set_hdmi_edid_ver(linein, ver);
    if(FPI_ERROR_SUCCESS == ret)
    {
        app_save_db_hdmi_version(ver, linein);
        DBG_SRC(" tos_source_set_hdmi_edid_ver, save ver = %d \n", ver);
    }
    return ret;
}
